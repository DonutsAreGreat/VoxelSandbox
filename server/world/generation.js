import { CHUNK_SIZE, MIN_HEIGHT, MAX_HEIGHT } from './constants.js';
  ...
  for (let y = 0; y < height; y++) {
    for (let z = 0; z < size; z++) {
      for (let x = 0; x < size; x++) {
        ...
        const id = isFrame ? MATERIAL.METAL : MATERIAL.WOOD;
        chunk.set(inX, inY, inZ, id);
      }
    }
  }
}
    ...
    for (let x = 0; x < CHUNK_SIZE; x++) {
      ...
      for (let y = 0; y < CHUNK_SIZE; y++) {
        ...
        if (wy < MIN_HEIGHT || wy > MAX_HEIGHT) {
          chunk.set(x, y, z, MATERIAL.AIR);
          continue;
        }
        if (wy === MIN_HEIGHT) {
          chunk.set(x, y, z, MATERIAL.BEDROCK);
          continue;
        }
        if (wy > h) {
          chunk.set(x, y, z, MATERIAL.AIR);
        ...
        const depth = h - wy;
        let id = MATERIAL.DIRT;
        if (depth > 3) id = MATERIAL.STONE;
        if (depth === 0) id = MATERIAL.GRASS;
        if (depth === 1 && hash2d(wx, wz) > 0.7) id = MATERIAL.WOOD;
